...
parser = argparse.ArgumentParser(description='PyTorch kaldi')
parser.add_argument('--cfg', type=str, default=None,
                    help='path to config file')
parser.add_argument('--admm-epochs', type=int, default=3, metavar='N',
...
# The run_nn is a function that process a single chunk of data  更改，也要更改对应的cfg

run_admm_script=config['exp']['run_admm_script'].split('.py')[0]
module = importlib.import_module('core_admm')
run_admm=getattr(module, run_admm_script)
run_admm_quant=getattr(module, 'run_admm_quant')
...
if args.admm_quant:
    ...
    valid_model_files_past=None
    valid_model_files=None
    ...
    for ep in range(N_ep):
        ...
        for tr_data in tr_data_lst:
            ...
            for ck in range(N_ck_tr):
                ...
                if not(os.path.exists(info_file)):
                        ...
                        [data_name,data_set,data_end_index,fea_dict,lab_dict,arch_dict]=run_admm_quant(args, data_name,data_set,data_end_index,fea_dict,lab_dict,arch_dict,config_chunk_file,processed_first,next_config_file,ep,ck)
                        ...
                        if not(os.path.exists(info_file)):
                            sys.stderr.write("ERROR: admming epoch %i, chunk %i not done! File %s does not exist.\nSee %s \n" % (ep,ck,info_file,log_file))
                            sys.exit(0)
        ...
        for valid_data in valid_data_lst:
            ...
            for ck in range(N_ck_valid):
                ...
                info_file=out_folder+'/exp_files/valid_'+valid_data+'_ep'+format(ep, N_ep_str_format)+'_trCk4_ck'+format(ck, N_ck_str_format)+'.info'
                config_chunk_file=out_folder+'/exp_files/valid_'+valid_data+'_ep'+format(ep, N_ep_str_format)+'_trCk4_ck'+format(ck, N_ck_str_format)+'.cfg'

                if valid_model_files_past == None:
                    valid_model_files_past={}
                else:
                    valid_model_files_past=valid_model_files

                valid_model_files={}
                for arch in pt_files.keys():
                    valid_model_files[arch]=info_file.replace('.info','_'+arch+'.pkl')
                ...
                # remove previous pkl files
                if len(valid_model_files_past.keys())>0:
                    for pt_arch in pt_files.keys():
                        if os.path.exists(valid_model_files_past[pt_arch]):
                            os.remove(valid_model_files_past[pt_arch])
    ...
    for pt_arch in pt_files.keys():
        if os.path.exists(valid_model_files[pt_arch]) and not os.path.exists(out_folder+'/exp_files/final_'+pt_arch+'.pkl'):
            copyfile(valid_model_files[pt_arch], out_folder+'/exp_files/final_'+pt_arch+'.pkl')
...
elif strtobool(config['exp']['train']):
    op_counter=1 # used to dected the next configuration file from the list_chunks.txt
...
elif strtobool(config['exp']['admm']):
    op_counter=1 # used to dected the next configuration file from the list_chunks.txt
        ...
        for tr_data in tr_data_lst:
            ...
            for ck in range(N_ck_tr):
                ...
                if not(os.path.exists(info_file)):
                        ...
                        # run chunk processing
                        [data_name,data_set,data_end_index,fea_dict,lab_dict,arch_dict,masks,ADMM]=run_admm(data_name,data_set,data_end_index,fea_dict,lab_dict,arch_dict,config_chunk_file,processed_first,next_config_file, ADMM, masks,ep,ck)
...
elif strtobool(config['exp']['retrain']):
    op_counter=1 # used to dected the next configuration file from the list_chunks.txt
        ...
        for tr_data in tr_data_lst:
            ...
            for ck in range(N_ck_tr):
                ...
                if not(os.path.exists(info_file)):
                        ...
                        if not(os.path.exists(info_file)):
                            sys.stderr.write("ERROR: retraining epoch %i, chunk %i not done! File %s does not exist.\nSee %s \n" % (ep,ck,info_file,log_file))
                            sys.exit(0)
